...
`mplot::GraphVisual` is a class that draws 2D graphs.

A short example of a program that draws a graph with `GraphVisual` is (omitting the `#include` lines for `<mplot/Visual.h>`, `<mplot/GraphVisual.h>` and `<sm/vvec>`):
```c++
int main() {
    sm::vvec<double> x = { -7,-6,-5,-4,-3,-2,-1,-0.5, 0, 0.5, 1, 2, 3, 4, 5, 6, 7  }; // data
    mplot::Visual v(1024, 768, "Made with mplot::GraphVisual"); // Visual scene/window
...
![Screenshot of a default GraphVisual graph of y = x^3](https://github.com/sebsjames/mathplot/blob/main/docs/images/graph_basic1.png?raw=true)
...
It has used the default font ([VisualFont](/mathplot/ref/visualint/visualface#available-font-faces)::DVSans) and font size (0.05) for the axis labels and tick labels.
The data markers have the default colour of `mplot::colour::royalblue` and lines of a default thickness (0.03) are drawn between the markers in black.
...
![Screenshot of two GraphVisual graphs, each showing two datasets](https://github.com/sebsjames/mathplot/blob/main/docs/images/graph_basic2.png?raw=true)
...
Each dataset in a GraphVisual has an associated `mplot::DatasetStyle`.
...
```c++
    mplot::DatasetStyle ds; // Equivalent: mplot::DatasetStyle ds (mplot::stylepolicy::both);
    ds.datalabel = "x cubed";
    ds.markercolour = mplot::colour::springgreen3;
    ds.markersize = ds.markersize * 1.5;
    ds.markerstyle = mplot::markerstyle::pentagon;
    gv->setdata (x, x.pow(3), ds);
```
Create a `DatasetStyle` instance, `ds`, then change some of its members. Here, we set the datalabel, changed the [colour](/mathplot/ref/visual/colour) for the markers and increased the marker size. Here's the resulting graph:

![Screenshot of two GraphVisual graphs, each showing two datasets](https://github.com/sebsjames/mathplot/blob/main/docs/images/graph_datasetstyle.png?raw=true)
...
You can find DatasetStyle in the header [mplot/DatasetStyle.h](https://github.com/sebsjames/mathplot/blob/main/mplot/DatasetStyle.h).

The public member attributes of `mplot::DatasetStyle` that you can modify are:

* `mplot::stylepolicy stylepolicy` Should we plot markers, lines or both? Options are `markers`, `lines`, `both` (coloured markers, black lines) `allcolour` (coloured markers and lines) or `bar` (bar graph). The default is `both`; if you want an alternative, pass it to the DatasetStyle constructor.
* `float linewidth` Thickness of lines in model units. Default is 0.007.
* `std::array<float, 3> linecolour` The [colour](/mathplot/ref/visual/colour) of the dataset line. RGB values in range [0,1].
...
* `mplot::markerstyle markerstyle` The shape of the data markers. `square`, `triangle`, `hexagon`, etc. Options in [graphstyles.h](https://github.com/sebsjames/mathplot/blob/main/mplot/graphstyles.h).
* `std::array<float, 3> markercolour` The [colour](/mathplot/ref/visual/colour) of the datum markers. RGB values in range [0,1].
...
```c++
    // Left hand graph
    mplot::DatasetStyle ds(mplot::stylepolicy::lines); // initialize for line graphs
    ds.datalabel = "x cubed";
    ds.linewidth = 0.014f;
...
![Screenshot of two GraphVisual graphs, with different line style options](https://github.com/sebsjames/mathplot/blob/main/docs/images/graph_lines.png?raw=true)

### Marker-only graph examples
Create a `DatasetStyle` with `mplot::stylepolicy::markers` and then change the defaults for `markersize`, `markerstyle` and `markercolour`.
```c++
    // Left hand graph
    mplot::DatasetStyle ds(mplot::stylepolicy::markers); // initialize for markers only
    ds.datalabel = "x cubed";
    ds.markersize = 0.06f;
    ds.markerstyle = mplot::markerstyle::diamond;
    ds.markercolour = mplot::colour::purple2;
    gv->setdata (x, x.pow(3), ds); // line will be the default colour of black
    ...
    ds.markercolour = mplot::colour::magenta;
    gv2->setdata (x, 5.0 * x.pow(2), ds);
```
When you choose `mplot::stylepolicy::markers`, your datasets are plotted with a shaped marker at each datum. The left hand example has purple squares, the right had has magenta hexagons.

![Screenshot of two GraphVisual graphs, with different marker style options](https://github.com/sebsjames/mathplot/blob/main/docs/images/graph_markers.png?raw=true)
...
```c++
    // Left hand graph
    mplot::DatasetStyle ds(mplot::stylepolicy::both); // equivalent to: mplot::DatasetStyle ds;
    ds.datalabel = "x cubed";
    ds.linewidth = 0.0035f;
    ds.linecolour = mplot::colour::purple2;
    ds.markersize = 0.06f;
    ds.markerstyle = mplot::markerstyle::diamond;
    ds.markercolour = mplot::colour::magenta;
    ds.markergap = 0.04;
    ...
    ds.datalabel = "5 times x squared";
    ds.linewidth = 0.012f;
    ds.linecolour = mplot::colour::navy;
    ds.markersize = 0.014f;
    ds.markerstyle = mplot::markerstyle::uphexagon;
    ds.markercolour = mplot::colour::dodgerblue;
    ds.markergap = 0.02;
    gv2->setdata (x, 5.0 * x.pow(2), ds);
```
![Screenshot of two GraphVisual graphs, with different line and marker style options](https://github.com/sebsjames/mathplot/blob/main/docs/images/graph_both.png?raw=true)
...
The example [graph_line.cpp](https://github.com/sebsjames/mathplot/blob/main/examples/graph_line.cpp) has well annotated code that shows how to use these functions. The simplest code is:
...
```c++
    ...
    sm::vvec<double> xcross = gv->add_y_crossing_lines (x, y, 7.0, ds);
```
...
![Screenshot of GraphVisual graph, with annotation lines](https://github.com/sebsjames/mathplot/blob/main/docs/images/graph_line_simple.png?raw=true)
...
![Screenshot of GraphVisual graph, with more annotation lines](https://github.com/sebsjames/mathplot/blob/main/docs/images/graph_line_full.png?raw=true)

To draw annotation lines where the data cross at particular *x* values, use `GraphVisual::add_x_crossing_lines` in a similar way ([graph_line_xcross.cpp](https://github.com/sebsjames/mathplot/blob/main/examples/graph_line_xcross.cpp) is the example code).

## Unicode for special characters

You can incorporate unicode characters into your `DatasetStyle` datalabels and axis labels.
Use `mplot::unicode::toUtf8()` to generate a UTF-8 string, as documented in [Unicode text handling](/mathplot/ref/visual/unicode).
Any of the `string` attributes that you can set can be passed a string containing UTF-8 character codes. For example:

```c++
    using uc = mplot::unicode; // I usually shorten mplot::unicode for readable code
    ds.datalabel = "x cubed is usually written as x" + uc::toUtf8 (uc::ss3); // ss3: superscript 3
    gv->setdata (x, x.pow(3), ds);

```
![Screenshot of a GraphVisual graphs with a unicode datalabel](https://github.com/sebsjames/mathplot/blob/main/docs/images/graph_withunicode.png?raw=true)
Most of the codes are intuitively named; `unicode::alpha`, `unicode::beta`, `unicode::gamma` and so on. Consult the unicode header file [unicode.h](https://github.com/sebsjames/mathplot/blob/main/mplot/unicode.h) for the full list.
...
The default style is `mplot::axisstyle::box`, as shown in the previous examples.
...
![Screenshot of four graphs with different axis styles](https://github.com/sebsjames/mathplot/blob/main/docs/images/graph_fouraxes.png?raw=true)

The code to setup the top left graph starts like this:
```c++
    auto gv = std::make_unique<mplot::GraphVisual<float>>(sm::vec<float>{0,0,0});
    v.bindmodel (gv);
    gv->axisstyle = mplot::axisstyle::L; // That's all you have to do
    // ...rest of the setup
```
...
```c++
    ...
    gv->tickstyle = mplot::tickstyle::ticksin; // tickstyle::ticksout is the default
    // ...rest of the setup
```
...
```c++
    ...
    gv->axiscolour = mplot::colour::cobalt;
    // ...rest of the setup
```
...
![Screenshot of four graphs with coloured axes](https://github.com/sebsjames/mathplot/blob/main/docs/images/graph_fouraxes_clr.png?raw=true)
