...
namespace mplot
{
    ...
    "    vec3 vgamma = vec3(gamma, gamma, gamma);\n"
    "    vec3 ccolor = pow(color, vgamma);\n"
    "    if (greyscale > 0) {\n"
    "        float lum = (ccolor.x + ccolor.y + ccolor.z) * 0.3333;\n"
    "        vec3 gcolor = vec3(lum, lum, lum);\n"
    "        vertex.color = vec4(gcolor, alpha);\n"
    "    } else {\n"
    "        vertex.color = vec4(ccolor, alpha);\n"
    "    }\n"
    ...
    "            ccolor = pow(vec3(iparam[ippos_i + idx * 5], iparam[ippos_i + idx * 5 + 1], iparam[ippos_i + idx * 5 + 2]), vgamma);\n"
    "            vertex.color = vec4(ccolor, iparam[ippos_i + idx * 5 + 3]);\n"
