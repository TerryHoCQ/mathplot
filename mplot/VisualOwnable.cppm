...
export namespace mplot
{
    ...
    public:
        ...
        /*!
         * A single separating axis test for two boxes whose centres are c1 and c2 with sizes given
         * by x1/y1/z1 and x2/y2/z2.  Note that this does not return the distance you would have to
         * move back along the axis to separate (this information could be used to keep collided
         * boxes separate). Axis is renormalized at start
         */
        static bool separating_axis_test (sm::vec<>& axis,
        ...
        // Use the separating axis theorem for two boxes to find collision between two oriented bounding boxes
        static bool collision_detect (const sm::mat<float, 3, 4>& obb1, const sm::mat<float, 3, 4>& obb2)
        {
            // Face normals. Our obb_x/y/z axes
            const sm::vec<> c1 = obb1.col(0);
            const sm::vec<> x1 = obb1.col(1);
            const sm::vec<> y1 = obb1.col(2);
            const sm::vec<> z1 = obb1.col(3);
            const sm::vec<> c2 = obb2.col(0);
            const sm::vec<> x2 = obb2.col(1);
            const sm::vec<> y2 = obb2.col(2);
            const sm::vec<> z2 = obb2.col(3);

            sm::vec<> nx1 = x1;
            if (separating_axis_test (nx1, c1, x1, y1, z1, c2, x2, y2, z2)) { return false; }
            sm::vec<> ny1 = y1;
            if (separating_axis_test (ny1, c1, x1, y1, z1, c2, x2, y2, z2)) { return false; }
            sm::vec<> nz1 = z1;
            if (separating_axis_test (nz1, c1, x1, y1, z1, c2, x2, y2, z2)) { return false; }

            sm::vec<> nx2 = obb2.col(1);
            if (separating_axis_test (nx2, c1, x1, y1, z1, c2, x2, y2, z2)) { return false; }
            sm::vec<> ny2 = obb2.col(2);
            if (separating_axis_test (ny2, c1, x1, y1, z1, c2, x2, y2, z2)) { return false; }
            sm::vec<> nz2 = obb2.col(3);
            if (separating_axis_test (nz2, c1, x1, y1, z1, c2, x2, y2, z2)) { return false; }

            // Edge combinations (formed from face normal cross products)
            sm::vec<> cpx1x2 = x1.cross(x2);
            if (separating_axis_test (cpx1x2, c1, x1, y1, z1, c2, x2, y2, z2)) { return false; }
            sm::vec<> cpx1y2 = x1.cross(y2);
            if (separating_axis_test (cpx1y2, c1, x1, y1, z1, c2, x2, y2, z2)) { return false; }
            sm::vec<> cpx1z2 = x1.cross(z2);
            if (separating_axis_test (cpx1z2, c1, x1, y1, z1, c2, x2, y2, z2)) { return false; }

            sm::vec<> cpy1x2 = y1.cross(x2);
            if (separating_axis_test (cpy1x2, c1, x1, y1, z1, c2, x2, y2, z2)) { return false; }
            sm::vec<> cpy1y2 = y1.cross(y2);
            if (separating_axis_test (cpy1y2, c1, x1, y1, z1, c2, x2, y2, z2)) { return false; }
            sm::vec<> cpy1z2 = y1.cross(z2);
            if (separating_axis_test (cpy1z2, c1, x1, y1, z1, c2, x2, y2, z2)) { return false; }

            sm::vec<> cpz1x2 = z1.cross(x2);
            if (separating_axis_test (cpz1x2, c1, x1, y1, z1, c2, x2, y2, z2)) { return false; }
            sm::vec<> cpz1y2 = z1.cross(y2);
            if (separating_axis_test (cpz1y2, c1, x1, y1, z1, c2, x2, y2, z2)) { return false; }
            sm::vec<> cpz1z2 = z1.cross(z2);
            if (separating_axis_test (cpz1z2, c1, x1, y1, z1, c2, x2, y2, z2)) { return false; }
